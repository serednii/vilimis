...
import WebFormModal from "../Webs/WebFormModal";

const WebsListItem = ({onUpdate, web, projects, endCustomers, clients}) => {
    ...
    const [modalWebId, setModalWebId] = React.useState(0);
            ...
            <div role="presentation" onClick={() => {
                ...
                <div className="col-auto">
                    <div className="icon-shape icon-xs icon-shape-primary rounded p-1"
                         style={{width: "5rem"}}>
                        {isImg = false}
                        {"projectId" in web && web.projectId && projects.filter(project => project.id === web.projectId).map((project, project_index) => (
                ...
                <div className="col ms-n2">
                    <h3 className="fs-6 fw-bold mb-1">{web.name}</h3>
                    <p className="mb-0">
                        {"projectId" in web && web.projectId && projects.filter(project => project.id === web.projectId).map((project, project_index) => (
                            <React.Fragment key={project_index}>
                                {project.name}
                    ...
                    {"deadLineDate" in web && web.deadLineDate && "date" in web.deadLineDate && (
                        <>
                            <div className="d-flex align-items-center">
                                ...
                                <span className="small">
                                                            {dateP1 > (new Date(web.deadLineDate.date)) ? (
                                                                <span style={{color: "red"}}> do 24 hodin - </span>
                                                            ) : dateP2 > (new Date(web.deadLineDate.date)) ? (
